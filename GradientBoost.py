...
if st.sidebar.button('Run Algorithm'):
    ...
    clf = GradientBoostingClassifier(criterion=criterion,loss=loss,max_depth=max_depth,random_state=42,min_samples_split=min_samples_split,min_samples_leaf=min_samples_leaf,max_features=max_features,max_leaf_nodes=max_leaf_nodes,min_impurity_decrease=min_impurity_decrease)
    ...
    plt.ylabel("Col2")
    orig = st.pyplot(fig)
    st.subheader("Accuracy for Gradient Boosting  " + str(round(accuracy_score(y_test, y_pred), 2)))
